...
class DASFdw(ForeignDataWrapper):
    ...
    def _raise_registration_failed(message, cause=None):
        ...
        raise MulticornException("Registration failed", code=ERROR, detail=json.dumps(error_struct))

    @staticmethod
    def _raise_unavailable(name, type, url, cause=None):
        error_struct = {'code': 'UNAVAILABLE', 'name': name, 'type': type, 'url': url}
        if cause:
            error_struct['cause'] = str(cause)
    ...
    def _raise_internal_error(message, cause=None):
        ...
        if cause:
            error_struct['cause'] = str(cause)
        raise MulticornException("Internal error", code=ERROR, detail=json.dumps(error_struct))
    ...
    def _health_check(stub: HealthCheckServiceStub):
        ...
        If the remote doesn't implement health-check, this may fail or raise UNIMPLEMENTED.
        """
    ...
    @staticmethod
    def _grpc_call_internal(das_name, das_type, das_url,
                            stub_caller, request, *,
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                if code == grpc.StatusCode.UNAVAILABLE:
                    # Out of attempts?
                    if attempt == attempts:
                        DASFdw._raise_unavailable(das_name, das_type, das_url, cause=e)
                    ...
                    if health_stub:
                        try:
                            ...
                            # If health-check fails, keep looping
                            continue
                    continue
...
def das_type_to_postgresql(t):
    ...
    if type_name == 'any':
        ...
        # Convert the inner type to PostgreSQL type, then add '[]'
        inner_type_str = das_type_to_postgresql(t.list.inner_type)
